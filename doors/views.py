...
def home(request):
    ...
    # Build category data — use icontains since category is now comma-separated
    category_data = []
...
def gallery(request):
    ...
    # Use icontains for comma-separated category/finish fields
    if category:
    ...
    if search:
        doors = doors.filter(Q(name__icontains=search) | Q(description__icontains=search))
    if price_max:
        try:
    ...
    price_range = Door.objects.aggregate(min_price=Min('price_min'), max_price=Max('price_max'))
...
def door_detail(request, pk):
    ...
    # Related doors — match any overlapping category using icontains on first category
    first_cat = door.get_category_list()[0] if door.get_category_list() else ''
    related_doors = Door.objects.filter(
        category__icontains=first_cat
    ).exclude(pk=pk)[:4] if first_cat else Door.objects.exclude(pk=pk)[:4]
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            inquiry.door = door
            inquiry.save()
            messages.success(request, 'Your inquiry has been received. We will contact you within 24 hours.')
...
def ai_preview(request):
    ...
    if request.method == 'POST':
        ...
        # Rectangle coordinates sent from the canvas (as % of displayed image)
...
def remove_white_background(door_img):
    """
    Remove white/near-white background from door image.
    Returns the door as BGRA (4-channel) with background made transparent.
    Works well for product photos on plain white/light backgrounds.
    """
    import cv2
    import numpy as np

    # Convert to BGRA so we can set alpha channel
    door_bgra = cv2.cvtColor(door_img, cv2.COLOR_BGR2BGRA)

    # ── Method: GrabCut + white threshold combined ─────────────────────

    # Step 1: Threshold — mark near-white pixels as background
    # Near-white = all channels > 220
    b, g, r = door_img[:,:,0], door_img[:,:,1], door_img[:,:,2]
    white_mask = ((b > 220) & (g > 220) & (r > 220)).astype(np.uint8) * 255

    # Step 2: Flood fill from all 4 edges to catch connected white background
    # (avoids removing white parts of the door itself, e.g. white painted door)
    ...
    # Pixels marked 128 are confirmed background (connected to edges)
    bg_mask = (flood_mask == 128).astype(np.uint8) * 255

    # Step 3: Dilate slightly to clean up fringe pixels
    kernel  = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
    bg_mask = cv2.dilate(bg_mask, kernel, iterations=1)

    # Step 4: Soft edge — erode bg_mask slightly and blur for anti-aliasing
    bg_soft = cv2.GaussianBlur(bg_mask.astype(np.float32), (5, 5), 0)

    # Step 5: Set alpha — 0 where background, 255 where door
...
def process_door_preview(preview, rect):
    """
    Overlay the selected door image onto the house photo.
    rect: dict with x, y, w, h as PERCENTAGES (0.0–100.0) of the actual image size.
    White background is removed from the door before overlaying.
    """
    try:
        import cv2
        import numpy as np
        ...
        # ── Step 1: Remove white background from door ─────────────────────
        door_bgra = remove_white_background(door_img)

        # ── Step 2: TIGHT CROP — remove empty transparent space ───────────
        # Find the bounding box of non-transparent pixels so left/right
        # white padding doesn't cause the door to appear small & off-centre
        alpha_ch = door_bgra[:, :, 3]
        non_transparent = np.where(alpha_ch > 10)  # threshold to ignore fringe
        if len(non_transparent[0]) > 0 and len(non_transparent[1]) > 0:
            ...
            # Add a tiny 2px padding so edges aren't clipped
            pad = 2
        ...
        # ── Step 3: Resize door (with alpha) to fill the rectangle ────────
        door_resized = cv2.resize(door_bgra, (pw, ph), interpolation=cv2.INTER_LANCZOS4)

        # ── Step 4: Brightness match ──────────────────────────────────────
        ...
        # Only use door pixels (non-transparent) for brightness sampling
        ...
        # ── Step 5: Alpha composite — door over house using door's alpha ──
        # alpha_3ch shape: (ph, pw, 3)
        alpha_3ch = np.stack([door_alpha, door_alpha, door_alpha], axis=2)

        roi_f  = roi.astype(np.float32)
        door_f = door_adjusted.astype(np.float32)

        # Composite: door where alpha=1, house where alpha=0
